--Ranger of the Light Bless
--Scripted by KillerxG
local s,id=GetID()
function s.initial_effect(c)
	--(1)Increase Level by 1, redirect if used as Synchro Material, both players gain 200 LP
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_RECOVER+CATEGORY_LVCHANGE)
...
function s.dktg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,PLAYER_ALL,200)
end
...
function s.drcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsLocation(LOCATION_GRAVE) and rc:IsCode(777003790) and r & REASON_SYNCHRO == REASON_SYNCHRO
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsCode(id) and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,777003790),tp,LOCATION_MZONE,0,1,nil) 
			and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
		local tg=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsCode,777003790),tp,LOCATION_MZONE,0,nil)
...
function s.rmcfilter(c,tp)
	return c:IsMonster() and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsPreviousLocation(LOCATION_MZONE)
end
