--Monk of the Fire Duality
--Scripted by KillerxG
local s,id=GetID()
function s.initial_effect(c)
	--(1)Reduce Level by 1, redirect if used as Synchro Material, both players lose 200 LP
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_DAMAGE+CATEGORY_LVCHANGE)
...
function s.dkfilter(c)
	return c:IsLevelAbove(2) and c:IsAttribute(ATTRIBUTE_FIRE)
end
...
function s.drcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsLocation(LOCATION_GRAVE) and rc:IsCode(777003810) and r & REASON_SYNCHRO == REASON_SYNCHRO
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsCode(id) and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,777003810),tp,LOCATION_MZONE,0,1,nil) 
			and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
		local tg=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsCode,777003810),tp,LOCATION_MZONE,0,nil)
