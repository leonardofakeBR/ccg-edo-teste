...
function s.spfilter1(c,tp)
	return c:IsSetCard(0x445) and not c:IsCode(id) and c:IsAbleToRemoveAsCost()
end
...
function s.hspcon(e,c)
	...
	local g1=Duel.GetMatchingGroup(s.spfilter1,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil)
	local g=g1:Clone()
...
function s.hsptg(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g1=Duel.GetMatchingGroup(s.spfilter1,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil)
	local g=aux.SelectUnselectGroup(g1,e,tp,1,1,s.rescon,1,tp,HINTMSG_REMOVE,nil,nil,true)
	if #g>0 then
		g:KeepAlive()
		e:SetLabelObject(g)
		return true
...
function s.shfilter(c)
	return c:IsSetCard(0x445b) and c:IsAbleToHand() and c:IsSpellTrap()
end
