...
function s.filter2(c,lv,code)
	return c:IsSetCard(0x304) and not c:IsCode(id) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if b and Duel.CheckReleaseGroupCost(tp,Card.IsAttribute,1,true,nil,nil,ATTRIBUTE_DARK)
		and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		local g=Duel.SelectReleaseGroupCost(tp,Card.IsAttribute,1,1,true,nil,nil,ATTRIBUTE_DARK)
